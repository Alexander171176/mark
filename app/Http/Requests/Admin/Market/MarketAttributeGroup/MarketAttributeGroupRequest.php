...
namespace App\Http\Requests\Admin\Market\MarketAttributeGroup;
...
class MarketAttributeGroupRequest extends FormRequest
{
    ...
    {
        $groupId = $this->route('marketAttributeGroup')?->id
            ?? $this->route('marketAttributeGroup')
            ?? $this->route('id');
        ...
        return [
                ...
                'code' => [
                    ...
                    'max:100',
                    'regex:/^[a-z0-9]+(?:-[a-z0-9]+)*$/',
                    Rule::unique('market_attribute_groups', 'code')->ignore($groupId),
                ],
    ...
    {
        return [
            'user_id.required' => 'Необходимо указать создателя группы характеристик.',
            ...
            'code.unique' => 'Группа характеристик с таким системным кодом уже существует.',

            'color.max' => 'Цвет группы не должен превышать 50 символов.',
            ...
            'translations.*.title.required' => 'Название группы обязательно для каждой добавленной локали.',
            'translations.*.title.max' => 'Название группы не должно превышать 255 символов.',
