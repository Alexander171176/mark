...
namespace App\Http\Requests\Admin\Market\MarketShop;
...
class MarketShopRequest extends FormRequest
{
    ...
    {
        $shopId = $this->route('marketShop')?->id
            ?? $this->route('marketShop')
            ?? $this->route('id');
        ...
        return [
                ...
                'market_company_id' => [
                    ...
                    Rule::unique('market_shops', 'market_company_id')
                        ->ignore($shopId),
                ],
                ...
                'images.*.id' => [
                    'nullable',
                    'integer',
                    Rule::exists('market_shop_images', 'id'),
                ...
                'deletedImages.*' => ['integer', 'exists:market_shop_images,id'],
    ...
    public function messages(): array
    {
        return [
            /* Основные поля */
            'market_company_id.required' => 'Необходимо выбрать компанию магазина.',
            'market_company_id.exists' => 'Указанная компания не найдена.',
            'market_company_id.unique' => 'У этой компании уже есть магазин.',

            'user_id.required' => 'Необходимо указать создателя магазина.',
            ...
            'url.unique' => 'Магазин с таким URL уже существует.',

            'email.email' => 'Email магазина имеет неверный формат.',
            'email.max' => 'Email магазина не должен превышать 255 символов.',
            'phone.max' => 'Телефон магазина не должен превышать 100 символов.',
            ...
            /* Изображения */
            'images.*.id.exists' => 'Одно из изображений магазина не найдено.',
            'images.*.id.prohibited_if' => 'При создании магазина нельзя передавать существующий ID изображения.',
            ...
            'translations.*.title.required' => 'Название магазина обязательно для каждой добавленной локали.',
            'translations.*.title.max' => 'Название магазина не должно превышать 255 символов.',
