...
class SchoolOrderResource extends JsonResource
{
    public function toArray(Request $request): array
    {
        return [
            ...
            'is_refunded' => in_array($this->payment_status, ['refunded', 'partial', 'partially_refunded'], true),
            ...
            'items' => SchoolOrderItemResource::collection(
                $this->whenLoaded('orderItems')
            ),

            'payments' => $this->whenLoaded('payments', fn () => $this->payments->map(fn ($payment) => [
                'id' => $payment->id,
                'school_payment_method_id' => $payment->school_payment_method_id,
                'school_user_payment_method_id' => $payment->school_user_payment_method_id,
                ...
                'refunded_amount' => $payment->refunded_amount !== null ? (string) $payment->refunded_amount : null,
            ...
            'enrollments' => $this->whenLoaded('enrollments', fn () => $this->enrollments->map(fn ($enrollment) => [
                ...
                'school_course_id' => $enrollment->school_course_id,
                'school_course_schedule_id' => $enrollment->school_course_schedule_id,
            ...
            'subscriptions' => $this->whenLoaded('subscriptions', fn () => $this->subscriptions->map(fn ($subscription) => [
                ...
                'school_subscription_plan_id' => $subscription->school_subscription_plan_id,
            ...
            'refunds_count' => $this->when(isset($this->refunds_count), fn () => (int) $this->refunds_count),
            'invoices_count' => $this->when(isset($this->invoices_count), fn () => (int) $this->invoices_count),
