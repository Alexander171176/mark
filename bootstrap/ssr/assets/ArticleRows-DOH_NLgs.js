  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(normalizedArticles.value, (article, index) => {
        var _a;
        _push(`<div itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem" class="group flex flex-col sm:flex-row gap-3 rounded-md border border-gray-200 bg-white shadow-sm p-3 transition hover:shadow-md dark:border-gray-700 dark:bg-gray-900"><meta itemprop="position"${ssrRenderAttr("content", index + 1)}><article itemprop="item" itemscope itemtype="https://schema.org/BlogPosting"${ssrRenderAttr("itemid", getShowRoute(article))} class="contents"><meta itemprop="url"${ssrRenderAttr("content", getShowRoute(article))}><meta itemprop="headline"${ssrRenderAttr("content", getTitle(article))}>`);
        if (getShort(article)) {
        ...
        _push(`<div class="min-w-0 flex-1 flex flex-col justify-around"><div class="flex items-start justify-center gap-3">`);
        _push(ssrRenderComponent(unref(Link), {
          href: getShowRoute(article),
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`<span itemprop="headline" class="truncate text-lg font-semibold text-slate-900/85 dark:text-slate-100/85 group-hover:opacity-75"${_scopeId}>${ssrInterpolate(getTitle(article))}</span>`);
            } else {
              return [
                createVNode("span", {
                  itemprop: "headline",
        ...
        _push(`</div>`);
        if (getShort(article)) {
          _push(`<div itemprop="description" class="mt-2 line-clamp-2 text-sm text-slate-700 dark:text-slate-300">${ssrInterpolate(getShort(article))}</div>`);
        } else {
          _push(`<!---->`);
        }
        if ((article == null ? void 0 : article.owner) || getAuthorName(article)) {
          ...
          _push(`<div class="text-xs font-semibold text-slate-700/85 dark:text-slate-300/85 truncate">${ssrInterpolate(getAuthorName(article))}</div></div>`);
        } else {
          _push(`<!---->`);
        }
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Components/Public/Default/Blog/BlogArticle/ArticleRows.vue");
