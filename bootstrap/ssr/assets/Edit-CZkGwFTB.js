...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const form = useForm({
      _method: "PUT",
      school_course_id: props.price.school_course_id ?? ((_a = props.price.course) == null ? void 0 : _a.id) ?? null,
    ...
    const getCourseTitle = (course) => {
      var _a2;
      return ((_a2 = course == null ? void 0 : course.translation) == null ? void 0 : _a2.title) || (course == null ? void 0 : course.slug) || `#${course == null ? void 0 : course.id}`;
    };
    ...
    const pageTitle = computed(() => {
      var _a2, _b2;
      return ((_b2 = (_a2 = props.price.course) == null ? void 0 : _a2.translation) == null ? void 0 : _b2.title) || getCourseTitle(
        props.price.course
      ) || `ID: ${props.price.id}`;
    });
    ...
    const submitForm = () => {
      form.transform((data) => ({
        ...data,
        school_course_id: toNullableId(
          data.school_course_id
        ),
        currency_id: toNullableId(
          data.currency_id
        ),
        price: toMoneyString(
          data.price
        ),
        sale_price: toMoneyString(
          data.sale_price
        ),
        compare_at_price: toMoneyString(
          data.compare_at_price
        ),
        starts_at: toDateTimeStringOrNull(
          data.starts_at
        ),
        ends_at: toDateTimeStringOrNull(
          data.ends_at
        ),
        activity: Boolean(
          data.activity
      ...
      form.post(
        route(
          ...
          {
            schoolCoursePrice: props.price.id
          }
        ),
        {
          preserveScroll: true,
          ...
          if (_push2) {
            _push2(ssrRenderComponent(TitlePage, null, {
              default: withCtx((_2, _push3, _parent3, _scopeId2) => {
                if (_push3) {
                  _push3(`${ssrInterpolate(unref(t)("editCoursePrice"))} - ${ssrInterpolate(pageTitle.value)} [ID: ${ssrInterpolate(__props.price.id)}] `);
                } else {
                  return [
                    createTextVNode(toDisplayString(unref(t)("editCoursePrice")) + " - " + toDisplayString(pageTitle.value) + " [ID: " + toDisplayString(__props.price.id) + "] ", 1)
                  ];
                }
              }),
              _: 1
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Pages/Admin/School/SchoolCoursePrices/Edit.vue");
