...
_sfc_main$3.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Components/Admin/System/User/Sort/SortSelect.vue");
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (__props.users.length > 0) {
        ...
        ssrRenderList(__props.users, (user) => {
          _push(`<tr class="text-sm font-semibold border-b-2"><td class="px-2 first:pl-5 last:pr-5 py-1 whitespace-nowrap"><div class="text-center text-slate-800 dark:text-slate-200">${ssrInterpolate(user.id)}</div></td><td class="px-2 first:pl-5 last:pr-5 py-1 whitespace-nowrap text-teal-600 dark:text-violet-200 hover:bg-slate-100 dark:hover:bg-cyan-800"><div class="text-left">${ssrInterpolate(user.name)}</div></td><td class="px-2 first:pl-5 last:pr-5 py-1 whitespace-nowrap text-blue-600 dark:text-blue-200 hover:bg-slate-100 dark:hover:bg-cyan-800"><div class="text-left">${ssrInterpolate(user.email)}</div></td><td class="px-2 first:pl-5 last:pr-5 py-1 whitespace-nowrap"><div class="flex flex-wrap gap-1"><!--[-->`);
          ...
          _push(ssrRenderComponent(_sfc_main$5, {
            onClick: ($event) => emit("delete", user.id, user.name)
          }, null, _parent));
          _push(`</div></td></tr>`);
        });
        _push(`<!--]--></tbody></table>`);
...
_sfc_main$2.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Components/Admin/System/User/Table/UserTable.vue");
...
const _sfc_main$1 = {
  ...
  setup(__props, { emit: __emit }) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "bg-white dark:bg-slate-700 shadow-lg rounded-sm border border-slate-400 dark:border-slate-500 relative" }, _attrs))}><div class="flex items-center justify-between px-3 py-2 border-b border-slate-400 dark:border-slate-500"><div class="text-xs text-slate-600 dark:text-slate-200">${ssrInterpolate(unref(t)("users"))}: ${ssrInterpolate(__props.users.length)}</div></div>`);
      if (__props.users.length) {
        _push(`<div class="p-3"><div class="grid gap-3 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4"><!--[-->`);
        ssrRenderList(__props.users, (user) => {
          _push(`<article class="relative flex flex-col h-full rounded-md border border-slate-400 dark:border-slate-500 bg-slate-50/70 dark:bg-slate-800/80 shadow-sm hover:shadow-md transition-shadow duration-150"><header class="flex items-center justify-between px-2 py-1 border-b border-dashed border-slate-400 dark:border-slate-500"><div class="flex flex-row items-center space-x-2"><span class="text-[10px] font-semibold px-1.5 py-0.5 rounded-sm border border-gray-400 bg-slate-200 dark:bg-slate-700 text-slate-800 dark:text-blue-100"> ID: ${ssrInterpolate(user.id)}</span><span class="text-[10px] text-blue-800 dark:text-blue-200 break-all"${ssrRenderAttr("title", user.email)}>${ssrInterpolate(user.email)}</span></div></header><div class="relative w-full h-24 flex items-center justify-center bg-slate-200 dark:bg-slate-900"><div class="h-16 w-16 rounded-full flex items-center justify-center bg-gradient-to-br from-sky-500 to-indigo-500 text-white text-xl font-bold shadow-md">${ssrInterpolate(getInitials(user.name))}</div></div><div class="flex flex-col flex-1 px-3 py-2 space-y-2"><div class="text-center"><div class="text-[13px] font-semibold text-teal-700 dark:text-teal-200 line-clamp-2"${ssrRenderAttr("title", user.name)}>${ssrInterpolate(user.name)}</div></div><div class="text-center"><div class="flex flex-wrap justify-center gap-1"><!--[-->`);
          ssrRenderList(user.roles || [], (role) => {
            _push(`<span class="px-1.5 py-0.5 bg-slate-100 dark:bg-slate-800 border border-dashed border-gray-400 text-orange-500 dark:text-orange-200 text-[10px] rounded-sm">${ssrInterpolate(role.name)}</span>`);
          });
          _push(`<!--]-->`);
          if (!user.roles || user.roles.length === 0) {
          ...
          _push(`</div></div></div><footer class="px-3 py-2 border-t border-dashed border-slate-400 dark:border-slate-500"><div class="flex items-center justify-center space-x-2">`);
          _push(ssrRenderComponent(_sfc_main$4, {
            href: _ctx.route("admin.users.edit", user.id)
          }, null, _parent));
          _push(ssrRenderComponent(_sfc_main$5, {
...
_sfc_main$1.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Components/Admin/System/User/View/UserCardGrid.vue");
...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const usersList = computed(() => {
      ...
      if (Array.isArray((_a = props.users) == null ? void 0 : _a.data)) return props.users.data;
      return [];
    });
    ...
    const sortParam = ref(
      props.sortParam || props.adminSystemUsersDefaultSort || props.adminSortUsers || "idDesc"
    );
    ...
    const getRolesText = (user) => {
      const roles = Array.isArray(user == null ? void 0 : user.roles) ? user.roles : [];
      return roles.map((role) => (role == null ? void 0 : role.name) || "").filter(Boolean).join(" ");
    };
    const getPermissionsText = (user) => {
      const permissions = Array.isArray(user == null ? void 0 : user.permissions) ? user.permissions : [];
    ...
    const filteredUsers = computed(() => {
      ...
      filtered = filtered.filter((user) => {
        const searchableValues = [
          normalize(user == null ? void 0 : user.name),
          normalize(user == null ? void 0 : user.email),
          normalize(getRolesText(user)),
          normalize(getPermissionsText(user))
        ];
        return searchWords.value.every((word) => {
          if (/^\d+$/.test(word) && safeNumber(user == null ? void 0 : user.id) === Number(word)) {
            return true;
          }
    ...
    const displayedUsers = computed(() => {
      return props.useServerProcessing ? usersList.value : paginatedUsers.value;
    });
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            _push2(ssrRenderComponent(_sfc_main$7, {
              ...
              mode: __props.adminSystemUsersProcessingMode,
              "use-server-processing": __props.useServerProcessing,
              total: __props.usersCount
            }, null, _parent2, _scopeId));
            _push2(`</div>`);
            if (__props.usersCount && !__props.useServerProcessing) {
            ...
            if (__props.usersCount) {
              ...
              if (!__props.useServerProcessing) {
                ...
                _push2(ssrRenderComponent(_sfc_main$f, { pagination: __props.users }, null, _parent2, _scopeId));
              }
              _push2(`</div>`);
            } else {
              _push2(`<!---->`);
            ...
            return [
              createVNode("div", { class: "px-2 py-2 w-full max-w-12xl mx-auto" }, [
                createVNode("div", { class: "p-4 bg-slate-50 dark:bg-slate-700 border border-blue-400 dark:border-blue-200 overflow-hidden shadow-md shadow-gray-500 dark:shadow-slate-400 bg-opacity-95 dark:bg-opacity-95" }, [
                  ...
                  __props.usersCount && !__props.useServerProcessing ? (openBlock(), createBlock(_sfc_main$8, {
                    key: 0,
                    modelValue: searchQuery.value,
                    "onUpdate:modelValue": ($event) => searchQuery.value = $event,
                    placeholder: unref(t)("searchByNameOrEmail")
                  }, null, 8, ["modelValue", "onUpdate:modelValue", "placeholder"])) : createCommentVNode("", true),
                  __props.usersCount && __props.useServerProcessing ? (openBlock(), createBlock(_sfc_main$9, {
                    key: 1,
                    modelValue: searchQuery.value,
                    "onUpdate:modelValue": ($event) => searchQuery.value = $event
                  ...
                  viewMode.value === "table" ? (openBlock(), createBlock(_sfc_main$2, {
                    ...
                    users: displayedUsers.value,
                    onDelete: confirmDelete
                  }, null, 8, ["users"])) : (openBlock(), createBlock(_sfc_main$1, {
                    key: 6,
                    users: displayedUsers.value,
                    onDelete: confirmDelete
                  }, null, 8, ["users"])),
...
_sfc_main.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Pages/Admin/System/Users/Index.vue");
