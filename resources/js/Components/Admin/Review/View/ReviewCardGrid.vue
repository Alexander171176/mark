 ...
 *
 * Карточное отображение универсальных отзывов.
 *
...
/** События родительской страницы Index.vue. */
...
const authorEmail = (review) => {
    return review?.author?.email || ''
}

/** Тип полиморфной сущности. */
...
<template>
    <div class="relative rounded-sm border border-slate-400 bg-white
                shadow-lg dark:border-slate-500 dark:bg-slate-700">
        ...
        <div v-if="reviews.length" class="p-3">
            <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <article v-for="review in reviews" :key="review.id"
                         class="relative flex h-full flex-col rounded-md border
                                border-slate-400 bg-slate-50/70 shadow-sm transition-shadow
                                duration-150 hover:shadow-md dark:border-slate-500
                    ...
                    <header
                        ...
                        <div
                            class="rounded-sm border border-gray-400 bg-slate-200 px-1.5 py-0.5
                                   text-[10px] font-semibold text-slate-800 dark:bg-slate-700
                                   dark:text-blue-100"
                    ...
                    <div class="flex flex-1 flex-col space-y-2 px-3 py-2">
                        ...
                        <div class="flex flex-col items-center justify-center">
                            <div class="break-words text-center font-semibold
                                        text-blue-700 dark:text-blue-300">
                                {{ authorName(review) }}
                            </div>

                            <div v-if="authorEmail(review)"
                                 class="break-all text-center text-[10px] font-semibold
                        ...
                        <div class="flex items-center justify-center gap-1"
                            ...
                            <span v-for="star in 5" :key="star" class="text-base leading-none"
                                  :class="star <= normalizedRating(review)
                                  ? 'text-amber-500' : 'text-slate-300 dark:text-slate-600'">
                                ★
                            </span>

                            <span class="ml-1 text-xs font-semibold
                                         text-slate-600 dark:text-slate-300">
                                {{ normalizedRating(review) }}/5
                            </span>
                        </div>

                        <!-- Текст отзыва -->
                        <button type="button"
                                class="block w-full text-left" :title="reviewText(review)"
                                @click="emit('view-details', review)">
                            <span class="line-clamp-4 text-xs font-semibold
                                         text-slate-600 dark:text-slate-200">
                                {{ reviewText(review) }}
                            </span>
                        </button>
                        ...
                        <div class="space-y-0.5 text-center">
                            ...
                            <div class="line-clamp-2 text-xs font-semibold
                            ...
                            <div v-if="reviewableBarcode(review)"
                                 class="text-[10px] text-slate-400 dark:text-slate-400">
                        ...
                        <div class="flex flex-wrap items-center justify-center gap-1">
                            ...
                            <span
                                class="rounded-sm border border-violet-300 bg-violet-100
                                       px-2 py-1 text-[10px] font-semibold text-violet-700
                            ...
                            <span
                                class="rounded-sm border border-cyan-300 bg-cyan-100
                                       px-2 py-1 text-[10px] font-semibold text-cyan-700
                        ...
                        <div class="flex justify-center">
                            <span class="rounded-sm border px-2 py-1 text-[10px] font-semibold"
                                  ...
                                  : 'border-slate-300 bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-300'">
                    ...
                    <!-- Действия -->
                    <footer class="border-t border-dashed border-slate-400 px-3 py-2
                                   dark:border-slate-500">
                        <div class="flex items-center justify-between gap-2">
                            <button type="button"
                                    class="flex items-center rounded border border-slate-300
                                           px-0 py-1 hover:border-blue-500 dark:border-blue-300
                                           dark:hover:border-blue-100"
                                    :title="t('view')" @click="emit('view-details', review)">
                                <svg class="mx-1 h-4 w-4 shrink-0 fill-current text-blue-500"
                                     viewBox="0 0 16 16">
                                    <path
                                        d="M5 9h11v2H5V9zM0 9h3v2H0v-2zm5 4h6v2H5v-2zm-5 0h3v2H0v-2zm5-8h7v2H5V5zM0 5h3v2H0V5zm5-4h11v2H5V1zM0 1h3v2H0V1z" />
                                </svg>
                            </button>
                            ...
                            <div class="flex items-center gap-1">
                                ...
                                <DeleteIconButton
                                    :title="t('delete')" @delete="emit('delete', review.id)" />
                            </div>
                        </div>
                    </footer>
                </article>
