...
const handleDragEnd = () => {
    const newOrderIds = localQuestions.value.map(question => question.id)
...
const toggleAll = (event) => {
    ...
    const ids = localQuestions.value.map(question => question.id)
                    ...
                    >
                        ...
                        <div class="flex flex-col flex-1 px-3 py-2 space-y-2 text-[11px]">
                            <div
                                ...
                                :title="question.quiz?.title || ('#' + question.school_quiz_id)"
                            >
                                {{ question.quiz?.title || ('Quiz ID: ' + question.school_quiz_id) }}
                            </div>
                            ...
                            <div
                                ...
                                :title="shortText(question.question_text, 300)"
                            >
                            ...
                            <div
                                ...
                                :title="shortText(question.explanation, 300)"
                            >
                                {{ shortText(question.explanation, 90) }}
                            </div>
                            ...
                            <div class="flex flex-wrap justify-between gap-2 mt-1 text-[10px]">
                                ...
                                >
                                    ...
                                    <span class="font-semibold">
                                        {{ getCorrectAnswersSummary(question) }}
                                    </span>
                                </span>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-1 text-[10px] text-center">
                                <span
                                    class="border border-dashed border-slate-300
                                           dark:border-slate-600 rounded-sm px-1"
                                >
                                    {{ t('answers') }}: {{ question.answers_count ?? 0 }}
                                </span>
                                ...
                                <span
                                    class="border border-dashed border-slate-300
                                           dark:border-slate-600 rounded-sm px-1"
                                >
                                    {{ t('attemptsLimit') }}: {{ question.attempt_items_count ?? 0 }}
                                </span>
                            </div>
                        </div>
                        ...
                        >
                            <div class="flex items-center space-x-1">
                                <ActivityToggle
                                    ...
                                    :title="question.activity ? t('enabled') : t('disabled')"
                                    @toggle-activity="emit('toggle-activity', question)"
                                />

                                <IconEdit
                                    :href="route('admin.schoolQuizQuestions.edit', {
