...
const images = computed(() => {
    const imgsRaw = Array.isArray(props.rubric?.images)
        ? props.rubric.images
        : (props.rubric?.images?.data ?? [])

    return (Array.isArray(imgsRaw) ? imgsRaw : [])
        .slice()
        .sort((x, y) => Number(x?.order ?? 0) - Number(y?.order ?? 0))
        .map((img, idx) => ({
            id: img?.id ?? `${props.rubric?.id ?? 'rubric'}-${idx}`,
            src: img?.url || img?.image_url || img?.src || img?.path || null,
            alt: img?.alt || props.rubric?.title || '',
            title: img?.title || img?.alt || props.rubric?.title || '',
...
const setCurrent = (idx) => {
    ...
    const i = Number(idx)
    currentIndex.value = Number.isFinite(i)
...
watch(
    () => [images.value.length, props.intervalMs],
    () => {
        currentIndex.value = 0
        start()
    }
