...
const getAssignmentTitle = (assignment) => {
    return getTranslation(assignment)?.title || ''
}

/** Краткое описание задания */
const getAssignmentShort = (assignment) => {
    return getTranslation(assignment)?.short || ''
}
...
const getCourseTitle = (assignment) => {
    return getCourse(assignment)?.title
        || t('course')
}

/** Название модуля */
const getModuleTitle = (assignment) => {
    return getModule(assignment)?.title
        || t('module')
}

/** Название урока */
const getLessonTitle = (assignment) => {
    return getLesson(assignment)?.title
        || t('lesson')
}
 ...
 * Форматирование даты.
 *
 * Локаль определяется выбранным
 * backend Public-переводом задания.
 */
const formatDate = (value, assignment) => {
    ...
    const locale =
        getTranslation(assignment)?.locale
        || undefined
    ...
    <div class="space-y-4">
        ...
        >
            ...
            <div class="min-w-0 flex-1 flex flex-col justify-around">
                ...
                <!-- Краткое описание -->
                <div
                    v-if="getAssignmentShort(assignment)"
                ...
                <!-- Данные -->
                <div
                    class="mt-3 flex flex-wrap items-center justify-center
                ...
                >
                    ...
                    >
                        ...
                        {{ t('publishedAt') }}:
                        {{ formatDate(assignment.published_at, assignment) }}
                    </span>
                ...
                >
                    ...
                    >
                        ...
                        {{ t('dueAt') }}:
                        {{ formatDate(assignment.due_at, assignment) }}
                    </div>

                    <Link
                        :href="assignmentLink(assignment)"
                        class="w-1/2 flex items-center justify-center
                               gap-2 rounded-sm px-3 py-1 btn-default"
                    >
