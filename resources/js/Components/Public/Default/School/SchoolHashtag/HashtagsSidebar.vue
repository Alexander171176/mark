...
<template>
    ...
    <div v-if="list.length" class="flex flex-wrap items-center gap-2">
        <Link
            ...
            :href="route('public.schoolHashtags.show', { slug: unwrap(hashtag).slug })"
            class="flex items-center justify-start gap-2 rounded-md
                   border border-gray-400 px-3 py-1 text-xs font-semibold
                   text-gray-700 transition hover:bg-slate-200
