...
const trackLink = (track) => {
    return track?.slug
        ? route('public.schoolTracks.show', { slug: track.slug })
        : '#'
}
...
const childrenCount = (track) => {
    return Number(track?.children_count ?? 0)
}

/** Количество курсов */
const coursesCount = (track) => {
    return Number(track?.courses_count ?? 0)
}
</script>
    ...
    >
        ...
        >
            ...
            >
                <meta
                    itemprop="identifier"
                    :content="String(track.id)"
                >

                <meta
                    itemprop="url"
                    :content="trackLink(track)"
                >

                <meta
                    itemprop="name"
                    :content="getTrackName(track)"
                >

                <meta
                    v-if="getTrackShort(track)"
                    itemprop="description"
                    :content="getTrackShort(track)"
                >

                <!-- Изображения -->
                <Link :href="trackLink(track)">
                ...
                <div class="flex flex-1 flex-col p-4">
                    ...
                    <div class="flex items-center justify-center text-center">
                        <Link
                            ...
                            :title="`${t('courses')}: ${coursesCount(track)}`"
