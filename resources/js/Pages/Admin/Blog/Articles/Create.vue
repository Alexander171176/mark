 ...
 * @pseudonym Александр Косолапов <kosolapov1976@gmail.com>
 * Создание статьи (паттерн)
 */
...
/**
 * helpers для мультиселектов.
 */
const upperLocale = (v) => (v ? String(v).toUpperCase() : '-');

const ownerLabel = (item) => {
    // поддержка owner как relation (owner.name / owner.email)
    const owner =
        item?.owner?.name ||
        item?.owner?.email ||
        item?.user?.name ||
...
const baseName = (item) => String(item?.title ?? item?.name ?? '').trim() || '—';

const multiLabel = (item) => {
    if (!item) return '';
    const id = item?.id ?? '-';
    const owner = ownerLabel(item);
    const loc = upperLocale(item?.locale);

    // скобки отдельно + перед названием
        ...
        <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-12xl mx-auto">
            <div class="p-4 bg-slate-50 dark:bg-slate-700
                ...
                <form @submit.prevent="submitForm" enctype="multipart/form-data" class="p-3 w-full">
                    ...
                    <!-- Дата публикации, псевдоним автора -->
                    <div class="mb-3 flex flex-col lg:flex-row sm:justify-between sm:space-x-4">
                        ...
                        <!-- Автор -->
                        <div class="flex flex-col lg:flex-row items-center mb-2 lg:mb-0 flex-1">
                            <LabelInput for="pseudonym"
                                        :value="`${t('nickname')} / ${t('author')}`"
                            ...
                            <InputText
                                ...
                                class="w-full"
                            />
                            <InputError class="mt-1 sm:mt-0" :message="form.errors.pseudonym"/>
                        </div>
                    </div>
                    ...
                    <!-- Мультиселект для рекомендованных видео -->
                    <div class="mb-3 flex flex-col items-start">
