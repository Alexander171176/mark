...
// Пропсы страницы (данные рубрики приходят с сервера)
...
/**
 * Шаблон перевода (для новых локалей)
 */
...
/**
 * Преобразование translations из API → в удобный объект
 * { ru: {...}, en: {...} }
 */
const buildTranslations = () => {
    ...
    const defaultLocale = props.targetLocale || props.currentLocale || 'ru'

    // если переводов нет — создаём дефолт
...
const form = useForm({
    ...
    deletedImages: [], // id удалённых изображений
})
...
/**
 * Заголовок страницы (динамический)
 */
const pageTitle = computed(() => {
    return currentTranslation.value.title
        || props.rubric.translation?.title
        || `ID: ${props.rubric.id}`
})

/**
 * Ошибка для текущей локали
 */
const getError = (key) => {
...
/**
 * Построение дерева родительских рубрик
 */
...
/**
 * Уже существующие изображения (из БД)
 */
...
/**
 * Новые изображения (ещё не сохранённые)
 */
const newImages = ref([])
...
<template>
    <AdminLayout :title="t('editRubric')">
        <template #header>
            <TitlePage>{{ t('editRubric') }}: {{ pageTitle }} [ID: {{props.rubric.id}}]</TitlePage>
        </template>
                    ...
                    >
                        ...
                        <div class="mb-3 flex flex-col items-start">
                            <LabelInput for="title">
                                <span class="text-red-500 dark:text-red-300 font-semibold">*</span>
                                {{ t('rubricTitle') }} [{{ activeLocale.toUpperCase() }}]
                            </LabelInput>
                            <InputText
                                id="title"
                                type="text"
