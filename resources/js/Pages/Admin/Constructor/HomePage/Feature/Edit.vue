...
/**
 * Feature Edit (Admin)
 * - Редактирование: FeatureSection + FeatureItem[]
 */
...
const props = defineProps({
    feature: { type: Object, required: true }, // FeatureSectionResource
    items:   { type: Array,  required: true }  // FeatureItemResource[]
})
...
const viewerLocale = ref(props.feature?.locale ?? 'ru')
const switching    = ref(false)

watch(viewerLocale, (loc) => {
    if (!loc || loc === props.feature?.locale) return
    router.get(
        route('admin.home-page.feature.sections.edit', { section: props.feature.id, locale: loc }),
        {},
        {
            preserveScroll: true,
            preserveState: false,
...
const sectionForm = useForm(mapSectionToForm(props.feature))
...
watch(() => props.feature, (s) => syncFormFromProps(s))

const updateSection = () => {
    sectionForm.put(
        route('admin.home-page.feature.sections.update', { section: props.feature.id }),
        {
            preserveScroll: true,
            onSuccess: () => {
                const fresh = page.props?.value?.feature
...
const buildItemForm = (item) => useForm({
    feature_section_id: item.feature_section_id ?? props.feature.id,
    title:       item.title ?? '',
...
const initItemForms = (list) => {
    ...
    // создадим заново
    ;(list || []).forEach((it) => { itemForms[it.id] = buildItemForm(it) })
}
...
const updateItem = (item) => {
    itemForms[item.id]
        ...
        .post(
            route('admin.home-page.feature.items.update', { item: item.id }),
            {
                preserveScroll: true,
                onError:  (errs) => console.error('FeatureItem update errors:', errs),
                onSuccess: () => { /* оставим значения, сбрасывать нечего */ },
            }
        )
}

// Небольшой helper-превью, если SVG отсутствует — покажем заглушку
...
<template>
    <AdminLayout :title="t('edit')">
        <template #header>
            <TitlePage>
                {{ t('pages') }}: {{ t('home') }} — Feature
                ({{ props.feature.locale?.toUpperCase() ?? 'RU' }})
            </TitlePage>
        </template>

        <div :key="props.feature.id" class="px-2 py-2 w-full max-w-12xl mx-auto">
            <div
                    ...
                    <div class="space-y-1">
                        <div class="mt-3 flex justify-between flex-col
                            ...
                            <div class="flex flex-row items-center gap-2">
                                ...
                                <LabelCheckbox for="is_dark" :text="t('isDark')" class="text-sm h-8 flex items-center" />
                            </div>

                            <!-- Сортировка секции -->
                            <div class="flex flex-row items-center gap-2">
                                <div class="h-8 flex items-center">
                                    <LabelInput for="feature-sort" :value="t('sort')" class="text-sm" />
                                </div>
                                <InputNumber id="feature-sort" type="number" v-model="sectionForm.sort" autocomplete="sort" class="w-24" />
                    ...
                    <div class="py-3 grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="space-y-3">
                            ...
                            <div>
                                ...
                                <InputText id="subtitle" type="text" v-model="sectionForm.subtitle" autocomplete="subtitle" />
                ...
                <div class="w-full mt-6">
                    <h2 class="pb-2 flex flex-row items-center border-b border-gray-400">
                        <svg class="shrink-0 h-4 w-4 mr-1" viewBox="0 0 24 24">
                            ...
                            />
                            <circle class="fill-current text-slate-700 dark:text-slate-300" cx="9" cy="8" r="2" />
                        </svg>
                        <span class="font-semibold text-md text-slate-700 dark:text-slate-300">
                          {{ t('cards') || 'Feature items' }}
                        </span>
                    </h2>
                        ...
                        >
                            ...
                            <div class="w-full md:w-1/2 space-y-3">
                                ...
                                <div>
                                    ...
                                    <InputError class="mt-2" :message="itemForms[item.id].errors.title" />
                                </div>

                                <div>
                                    <LabelInput :for="`item-subtitle-${item.id}`" :value="t('subtitle')" />
                                    <InputText :id="`item-subtitle-${item.id}`" type="text"
                                               v-model="itemForms[item.id].subtitle" />
                                    <InputError class="mt-2" :message="itemForms[item.id].errors.subtitle" />
                                </div>

                                <div>
                                    <LabelInput :for="`item-desc-${item.id}`" :value="t('description')" />
                                    <MetaDescTextarea :id="`item-desc-${item.id}`"
                                                      v-model="itemForms[item.id].description" />
                                    <InputError class="mt-2" :message="itemForms[item.id].errors.description" />
                                </div>
                            </div>
                            ...
                            <div class="w-full md:w-1/2 pt-2">
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    ...
                                    <div class="space-y-2">
                                        ...
                                        <CodeTextarea
                                            ...
                                            :placeholder="`<svg ...>...</svg>`"
                                        />
                                        <InputError class="mt-2" :message="itemForms[item.id].errors.image_dark" />
                                    </div>
                                </div>
                                ...
                                <div class="space-y-2 mt-1">
                                    ...
                                    <InputError class="mt-2" :message="itemForms[item.id].errors.alt" />
                                </div>
