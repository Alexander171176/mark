 ...
 * Редактирование прайса курса (CoursePrice) — по паттерну Edit.vue курсов
 */
 ...
 * return Inertia::render('Admin/CoursePrices/Edit', [
...
const props = defineProps({
    coursePrice: { type: Object, required: true },
...
const form = useForm({
    ...
    course_id: props.coursePrice.course_id ?? props.coursePrice.course?.id ?? null,
    currency_id: props.coursePrice.currency_id ?? props.coursePrice.currency?.id ?? null,

    price: props.coursePrice.price ?? '',
    sale_price: props.coursePrice.sale_price ?? '',
    compare_at_price: props.coursePrice.compare_at_price ?? '',

    // контроллер отдаёт ISO, input datetime-local принимает YYYY-MM-DDTHH:mm
    starts_at: '',
    ends_at: '',

    activity: Boolean(props.coursePrice.activity),
    sort: props.coursePrice.sort ?? 0,

    meta: props.coursePrice.meta ?? null,
})
...
/** init datetime-local */
form.starts_at = isoToLocalInput(props.coursePrice.starts_at)
form.ends_at = isoToLocalInput(props.coursePrice.ends_at)
...
const selectedCourse = ref(
    courseOptions.value.find(o => o.id === (props.coursePrice.course_id ??
        props.coursePrice.course?.id)) ?? null
)

/** выбранная валюта */
const selectedCurrency = ref(
    currencyOptions.value.find(o => o.id === (props.coursePrice.currency_id ??
        props.coursePrice.currency?.id)) ?? null
)
...
const submitForm = () => {
    ...
    form.post(route('admin.coursePrices.update', props.coursePrice.id), {
        preserveScroll: true,
        onSuccess: () => {
...
<template>
    <AdminLayout :title="t('editCoursePrice')">
        <template #header>
            <TitlePage>
                {{ t('editCoursePrice') }} - [ID: {{ props.coursePrice.id }}]
            </TitlePage>
        </template>
                        ...
                        >
                            <template #icon>
                                <svg class="w-4 h-4 fill-current text-slate-100" viewBox="0 0 16 16">
                                    <path
                                        d="M14.3 2.3L5 11.6 1.7 8.3c-.4-.4-1-.4-1.4 0-.4.4-.4 1 0 1.4l4 4c.2.2.4.3.7.3.3 0 .5-.1.7-.3l10-10c.4-.4.4-.4-1.4 0z"
                                    />
                                </svg>
                            </template>
                            {{ t('save') }}
