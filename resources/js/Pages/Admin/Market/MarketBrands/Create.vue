...
const submitForm = () => {
    ...
    form.post(route('admin.marketBrands.store'), {
        forceFormData: true,
        errorBag: 'createMarketBrand',
        preserveScroll: true,

        onSuccess: () => toast.success('Бренд успешно создан!'),
...
<template>
    <AdminLayout :title="t('addMarketBrand')">
        <template #header>
            <TitlePage>{{ t('addMarketBrand') }}</TitlePage>
        </template>
