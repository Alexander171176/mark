...
const form = useForm({
    user_id:
        ...
        || bundle.value.owner?.id
        || null,

    market_company_id:
        bundle.value.market_company_id
        || bundle.value.company?.id
        || null,

    market_shop_id:
        bundle.value.market_shop_id
        || bundle.value.shop?.id
        || null,

    currency_id:
        bundle.value.currency_id
        || bundle.value.currency?.id
        || null,
...
/** Инициализировать выбранные связи */
const initializeRelations = () => {
    selectedCompany.value =
        companyOptions.value.find((item) => {
            return Number(item.id)
                === Number(form.market_company_id)
        }) || null

    /*
     * Магазин ищем в полном списке, поскольку выбранная
     * компания ещё могла не успеть ограничить shopOptions.
     */
    selectedShop.value =
        shopList.value
            .map((item) => ({
    ...
    selectedCurrency.value =
        currencyOptions.value.find((item) => {
            return Number(item.id)
                === Number(form.currency_id)
        }) || null
}

/** Первичная инициализация связей */
initializeRelations()
...
watch(selectedCompany, (value) => {
    ...
    if (
        selectedShop.value
        && Number(
            selectedShop.value.market_company_id
        ) !== Number(value?.id)
    ) {
