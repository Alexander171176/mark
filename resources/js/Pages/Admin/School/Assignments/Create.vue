...
/**
 * Пропсы из AssignmentController@create:
 *  - courses
 *  - modules
 *  - lessons
 *  - instructors
...
const instructorOptions = computed(() =>
    props.instructors.map(i => {
        ...
        // Формат как у остальных: с ID в квадратных скобках
        // Пример: "[ID: 7] Иван Петров — Senior Laravel Dev [ivan@example.com]"
...
/** 🔹 Инструктор: multiselect -> form.instructor_profile_id */
...
watch(
    ...
    () => {
        ...
        // 2) если выбран урок и он не из этого модуля — сбрасываем урок
        if (form.lesson_id) {
            const lesson = selectedLessonEntity.value
...
const submitForm = () => {
    ...
    form.post(route('admin.assignments.store'), {
        ...
        onError: (errors) => {
            console.error('❌ Ошибка при создании Задания:', errors)
