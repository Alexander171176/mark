 ...
 * courses — список курсов для выбора в составе бандла
 *
...
/** Опции курсов для мультиселекта */
const courseOptions = computed(() =>
    props.courses.map(c => {
        ...
        return {
            id: c.id,
            label: c.title
                ? `[ID: ${c.id}] ${locale} ${c.title}`
                : `[ID: ${c.id}] ${locale}`,
        }
    })
)

/** Выбранные курсы (объекты) */
...
/** Отправка формы создания бандла */
const submitForm = () => {
    form.transform((data) => {
        return {
            ...data,
    ...
    form.post(route('admin.bundles.store'), {
        ...
        forceFormData: true,
        onSuccess: () => {
            toast.success('Бандл успешно создан!')
        },
        onError: (errors) => {
            console.error('❌ Ошибка при создании бандла:', errors)
            ...
            >
                ...
                <form @submit.prevent="submitForm" enctype="multipart/form-data" class="p-3 w-full">
                    ...
                    <div class="mb-3 flex flex-col items-start">
                        ...
                        <InputText id="title" type="text" v-model="form.title" required autocomplete="off" />
                    ...
                    <div class="mb-3 flex flex-col items-start">
                        ...
                        <InputText id="meta_title" type="text" v-model="form.meta_title" maxlength="160"
                                   autocomplete="off" />
