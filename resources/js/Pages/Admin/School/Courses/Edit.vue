 ...
 * course — ресурс курса с отношениями (instructor, rating, images)
 * instructorProfiles — список доступных инструкторов для селекта
 */
const props = defineProps({
    ...
    courses: { type: Array, default: () => [] }, // 🔹 новые курсы для "рекомендуемых"
})

/**
 * Формируем форму редактирования курса.
 */
const form = useForm({
    _method: 'PUT',
    instructor_profile_id: props.course.instructor_profile_id ?? props.course.instructorProfile?.id ?? null,
    ...
    locale: props.course.locale ?? '',
    title: props.course.title ?? '',
    slug: props.course.slug ?? '',
    subtitle: props.course.subtitle ?? '',
    short: props.course.short ?? '',
    ...
    meta_desc: props.course.meta_desc ?? '',
    // 🔹 новые поля для связей
    learning_category_ids: ((props.course.learning_categories ?? props.course.learningCategories) || []).map(c => c.id),
    hashtag_ids: (props.course.hashtags || []).map(t => t.id),
    // 🔹 новые ID рекомендуемых курсов
    related_course_ids: (
        (props.course.related_courses ?? props.course.relatedCourses) || []
    )
...
/** Текущее значение селекта (single). */
const selectedInstructorProfile = ref(
    (() => {
        const currentId = props.course.instructor_profile_id ?? props.course.instructorProfile?.id
        const found = props.instructorProfiles.find(i => i.id === currentId)

        if (!found) return null

        const locale   = found.locale || '—'
        const userName = found.user?.name || found.full_name || found.title || `#${found.id}`
        const title    = found.title ?? null

        let label = `[ID: ${found.id}] [${locale}] ${userName}`
...
/** Текущее значение мультиселектов категорий. */
const selectedLearningCategories = ref(
    ((props.course.learning_categories ?? props.course.learningCategories) || []).map(c => ({
        id: c.id,
        label: c.name || `#${c.id}`,
    }))
)

/** Текущее значение мультиселектов хештегов. */
const selectedHashtags = ref(
    (props.course.hashtags || []).map(t => ({
        id: t.id,
        label: t.name || `#${t.id}`,
        color: t.color || null,
    }))
...
/** Опции селекта рекомендуемых курсов. */
...
/** Текущее значение мультиселекта рекомендуемых курсов. */
const selectedRelatedCourses = ref(
    ((props.course.related_courses ?? props.course.relatedCourses) || [])
        .filter(c => c.id !== props.course.id)
        .map(c => ({
            id: c.id,
            label: `[ID: ${c.id}] [${c.locale || '—'}] ${c.title || `#${c.id}`}`,
        }))
)

/** синхронизируем IDs рекомендуемых курсов в форму  */
...
const existingImages = ref(
    (props.course.images || [])
        .filter(img => img.webp_url || img.url || img.image_url)
        .map(img => ({
            id: img.id,
            url: img.webp_url || img.url || img.image_url,
            order: img.order || 0,
            alt: img.alt || '',
            caption: img.caption || '',
        }))
...
/** Отправка формы обновления курса. */
const submitForm = () => {
    form.transform((data) => {
    ...
    form.post(route('admin.courses.update', props.course.id), {
        ...
        onError: (errors) => {
            console.error('❌ Ошибка при обновлении курса:', errors)
...
<template>
    <AdminLayout :title="t('editCourse')">
        <template #header>
            <TitlePage>
                {{ t('editCourse') }} - {{ props.course.title }} [ID: {{course.id}}]
            </TitlePage>
        </template>
        ...
        <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-12xl mx-auto">
            ...
            >
                ...
                >
                    ...
                    <!-- Показывать в левой, правой колонке и в главном -->
                    ...
                    <!-- Показывать в новинках, в рекомендованных, в распродаже -->
                    <div class="mb-3 flex justify-between flex-col lg:flex-row items-center gap-4">

                        <!-- Показывать в новинках -->
                        <div class="flex flex-row items-center gap-2">
                            <ActivityCheckbox v-model="form.is_new"/>
                            <LabelCheckbox for="left" :text="t('sortIsNew')"
                                           class="text-sm h-8 flex items-center"/>
                        </div>
                        ...
                        <div class="flex flex-row items-center gap-2">
                            ...
                            <LabelCheckbox for="main" :text="t('sortIsHit')"
                                           class="text-sm h-8 flex items-center"/>
                        </div>
                        ...
                        <div class="flex flex-row items-center gap-2">
                            ...
                            <LabelCheckbox for="right" :text="t('sortIsSale')"
                                           class="text-sm h-8 flex items-center"/>
                        </div>

                    </div>

                    <!-- Уровень, Доступность, Статус -->
                    ...
                    <div class="mb-3 flex flex-col items-start">
                        ...
                        <VueMultiselect
                            ...
                            class="w-full"
                        />
                        <InputError class="mt-2" :message="form.errors.learning_category_ids" />
                    </div>
