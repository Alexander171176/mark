 ...
 * Редактирование хештега (Hashtag)
 */
...
const form = useForm({
    ...
    locale: props.hashtag.locale ?? '',
    name: props.hashtag.name ?? '',
    slug: props.hashtag.slug ?? '',
    color: props.hashtag.color ?? '',
    short: props.hashtag.short ?? '',
    description: props.hashtag.description ?? '',
    meta_title: props.hashtag.meta_title ?? '',
    meta_keywords: props.hashtag.meta_keywords ?? '',
    meta_desc: props.hashtag.meta_desc ?? '',
...
const submitForm = () => {
    ...
    form.post(route('admin.hashtags.update', props.hashtag.id), {
        preserveScroll: true,
        forceFormData: true,
        onSuccess: () => {
            toast.success('Тег обучения успешно обновлен!')
        },
        onError: (errors) => {
            console.error("❌ Ошибка при обновлении тега обучения:", errors)
...
<template>
    <AdminLayout :title="t('editTag')">
        <template #header>
            <TitlePage>
                {{ t('editTag') }} -{{ props.hashtag.name }} [ID: {{hashtag.id}}]
            </TitlePage>
        </template>
            ...
            <div class="p-4 bg-slate-50 dark:bg-slate-700
                ...
                <form @submit.prevent="submitForm" class="p-3 w-full">
                    ...
                    <div class="flex items-center justify-center mt-4">
                        <DefaultButton :href="route('admin.hashtags.index')" class="mb-3">
                            <template #icon>
                                <svg class="w-4 h-4 fill-current text-slate-100 shrink-0 mr-2"
                                     viewBox="0 0 16 16">
                                    <path d="M4.3 4.5c1.9-1.9 5.1-1.9 7 0 .7.7 1.2 1.7 1.4 2.7l2-.3c-.2-1.5-.9-2.8-1.9-3.8C10.1.4 5.7.4 2.9 3.1L.7.9 0 7.3l6.4-.7-2.1-2.1zM15.6 8.7l-6.4.7 2.1 2.1c-1.9 1.9-5.1 1.9-7 0-.7-.7-1.2-1.7-1.4-2.7l-2 .3c-.2 1.5.9 2.8 1.9 3.8 1.4 1.4 3.1 2 4.9 2 1.8 0 3.6-.7 4.9-2l2.2 2.2 .8-6.4z"></path>
