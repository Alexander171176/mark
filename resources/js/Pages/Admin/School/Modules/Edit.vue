...
import SelectStatus from '@/Components/Admin/Module/Select/SelectStatus.vue'
import SelectAvailability from '@/Components/Admin/Module/Select/SelectAvailability.vue'
 ...
 * module — ресурс модуля с отношениями (courses, rating, images)
 */
...
const form = useForm({
    _method: 'PUT',
    course_id: props.module.course?.id ?? props.module.course_id ?? null,
    ...
    meta_title: props.module.meta_title ?? '',
    meta_keywords: props.module.meta_keywords ?? '',
    meta_desc: props.module.meta_desc ?? '',
    deletedImages: [],
})
...
/** Текущее значение мультиселекта модуля. */
const selectedCourse = ref(
    props.module.course
        ? {
            id: props.module.course.id,
            label: props.module.course.title || `#${props.module.course.id}`,
        }
        : null
)

/** синхронизируем в форму */
...
const submitForm = () => {
    ...
    form.post(route('admin.modules.update', props.module.id), {
        preserveScroll: true,
        forceFormData: true,
        onSuccess: () => {
            toast.success('Модуль обучения успешно обновлён!')
        },
        onError: (errors) => {
            console.error('❌ Ошибка при обновлении Модуля обучения:', errors)
...
<template>
    <AdminLayout :title="t('editModule')">
        <template #header>
            <TitlePage>
                {{ t('editModule') }}: {{ props.module.title }} [ID: {{module.id}}]
            </TitlePage>
        </template>
        ...
        <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-12xl mx-auto">
            ...
            >
                <div class="sm:flex sm:justify-between sm:items-center mb-2">
                    <!-- Кнопка назад -->
                    <DefaultButton :href="route('admin.modules.index')">
                        <template #icon>
                            <svg
