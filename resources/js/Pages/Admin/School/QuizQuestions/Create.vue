 ...
 * Создание вопроса квиза (QuizQuestion)
 */
...
/**
 * Пропсы из QuizQuestionController@create:
...
const form = useForm({
    ...
    // sort = 0 → если нужно, бэкенд может переопределить
    sort: 0,
    question_type: 'single_choice', // чтобы не было пустого значения
    question_text: '',
    explanation: '',
    points: 0,
    activity: false,
...
/**
 * Инициализация selectedQuiz на основе defaultQuizId.
 * ВАЖНО:
 *  - если defaultQuizId есть → выбираем этот квиз
 *  - если нет → ничего не выбираем, пользователь сам выбирает
 */
watchEffect(() => {
    ...
    // Если есть defaultQuizId — пытаемся найти соответствующий квиз
    if (props.defaultQuizId) {
        const found = options.find(o => o.id === props.defaultQuizId)
        ...
        // defaultQuizId нет — ничего не выбрано, юзер сам выбирает
        selectedQuiz.value = null
        form.quiz_id = null
    }
})
...
/**
 * Мапа: значение из БД → ключ перевода (как в Edit.vue и таблице)
 */
...
/**
 * Отправка формы создания вопроса.
 * meta_raw -> meta (JSON)
 */
const submitForm = () => {
    form.transform((data) => {
        ...
        if (meta_raw && meta_raw.trim()) {
            try {
                ...
                console.error('❌ Ошибка парсинга meta JSON при создании вопроса:', e)
                // meta остаётся null — бэкенд провалидирует
            }
        }
    ...
    form.post(route('admin.quizQuestions.store'), {
        preserveScroll: true,
        onSuccess: () => {
            toast.success('Вопрос квиза успешно создан!')
        },
        onError: (errors) => {
            console.error('❌ Ошибка при создании вопроса квиза:', errors)
        ...
        <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-6xl mx-auto">
            ...
            >
                ...
                <div class="sm:flex sm:justify-between sm:items-center mb-4">
                    <DefaultButton :href="route('admin.quizQuestions.index', { quiz_id: form.quiz_id })">
                        <template #icon>
                            <svg
                ...
                >
                    ...
                    <div class="flex items-center justify-center mt-6 gap-3">
                        <DefaultButton
                            :href="route('admin.quizQuestions.index', { quiz_id: form.quiz_id })"
                            class="mb-3"
                        >
                            <template #icon>
                                <svg
