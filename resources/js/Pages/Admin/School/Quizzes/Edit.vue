...
// Приведение ISO-строки к формату datetime-local
...
const form = useForm({
    ...
    module_id: props.quiz.module_id ?? null,
    lesson_id: props.quiz.lesson_id ?? null,
    ...
    locale: props.quiz.locale ?? (props.currentLocale || 'ru'),
    title: props.quiz.title ?? '',
    slug: props.quiz.slug ?? '',
    short: props.quiz.short ?? '',
    description: props.quiz.description ?? '',

    // Параметры квиза
    type: props.quiz.type ?? '',
    attempts_limit: props.quiz.attempts_limit ?? 0,
    time_limit_minutes: props.quiz.time_limit_minutes ?? 0,
    pass_score: props.quiz.pass_score ?? 0,

    published_at: normalizeDateTime(props.quiz.published_at),
    images: [],
...
watch(
    ...
    () => {
        ...
        if (!lesson) return // если урок сбросили — ничего не трогаем
        ...
        // 3) UI-синк multiselect объектов
        if (form.module_id) {
...
watch(
    ...
    () => {
        ...
        if (!module) return // если модуль сбросили — ничего не трогаем
...
/**
 * ✅ 3) При ручной смене курса — сбрасываем модуль/урок, если они не из этого курса
 * Важно: только при ручной смене, не при автоподстановке.
 */
watch(
    () => form.course_id,
    (newVal, oldVal) => {
...
/**
 * ✅ UI-синк: когда form.*_id меняется программно — обновляем multiselect-объекты
 */
watch(
    () => form.course_id,
    () => {
...
/** Отправка формы обновления квиза */
const submitForm = () => {
    form.transform((data) => {
        return {
            ...data,
    ...
    form.post(route('admin.quizzes.update', props.quiz.id), {
        ...
        onError: (errors) => {
            console.error('❌ Ошибка при обновлении Квиза:', errors)
...
<template>
    <AdminLayout :title="t('editQuiz')">
        <template #header>
            <TitlePage>
                {{ t('editQuiz') }} - {{quiz.title}} [ID: {{quiz.id}}]
            </TitlePage>
        </template>
