...
/**
 * При глобальном переключении locale
 * синхронизируем активную вкладку формы.
 */
watch(
    () => props.currentLocale,
    (locale) => {
...
/**
 * Controller передаёт SchoolCourseSharedResource,
 * где перевод текущей locale находится в:
 *
...
/**
 * form.course_ids — единственное состояние.
 *
 * Multiselect получает выбранные объекты
 * через computed и всегда использует свежие
 * locale-aware props.courses.
 */
const selectedCourses = computed({
    get: () => {
        const ids = Array.isArray(form.course_ids)
            ? form.course_ids
            : []

        return courseOptions.value.filter(course =>
            ids.some(
    ...
    set: (courses) => {
        form.course_ids = Array.isArray(courses)
...
const submit = () => {
    form.transform(data => {
        ...
        /**
         * images формируем вручную
         * для multipart FormData.
         */
