 ...
 * Props.
 *
 * Enrollment сам переводов не имеет.
 *
 * currentLocale влияет только на связанные:
 *
 * - courses;
 * - schedules;
 * - schedule.course.
 ...
 * Форма.
 *
 * Используем новый backend-контракт:
 *
 * school_course_id
...
const form = useForm({
    ...
    user_id:
        props.enrollment.user_id
        ?? null,

    school_course_id:
        props.enrollment.school_course_id
        ?? null,

    school_course_schedule_id:
        props.enrollment.school_course_schedule_id
        ?? null,

    school_order_id:
        props.enrollment.school_order_id
        ?? null,
    ...
    progress_percent:
        props.enrollment.progress_percent
        ?? 0,

    notes:
        props.enrollment.notes
        ?? '',

    meta:
        props.enrollment.meta
        ?? null,
})

/**
 * datetime-local модели.
 */
 ...
 * и т.д.
 *
 * Браузер сам понимает BCP-47 locale.
 */
...
/**
 * Получение title из нового
 * SharedResource-контракта.
 */
...
/**
 * При выборе потока автоматически
 * синхронизируем Course.
 *
 * При смене locale этот watcher
 * не срабатывает, потому что FK
 * schedule_id не меняется.
 */
watch(
    () =>
        form.school_course_schedule_id,
