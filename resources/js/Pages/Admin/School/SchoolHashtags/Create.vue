...
// ======================== Локаль ========================
const defaultLocale = props.currentLocale || 'ru' // локаль по умолчанию
const activeLocale = ref(defaultLocale)          // текущая вкладка языка

// ======================== Форма ========================
const form = useForm({
    activity: true,              // активность хештега
    sort: 0,                     // порядок сортировки
    slug: '',                    // URL slug
    color: '#FF6600',            // цвет

    // переводы (объект вида { ru: {...}, en: {...} })
    translations: {
...
const currentTranslation = computed(() => {
    // гарантируем существование перевода
...
const submit = () => {
    ...
    form.post(route('admin.schoolHashtags.store'), {
        errorBag: 'createSchoolHashtag',
        preserveScroll: true,
        forceFormData: true,
        onSuccess: () => {
            toast.success('Хештег успешно создан!')
        },
        onError: (errors) => {
