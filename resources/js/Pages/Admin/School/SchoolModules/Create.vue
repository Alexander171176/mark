...
import SelectStatus from '@/Components/Admin/School/SchoolModule/Select/SelectStatus.vue'
import SelectAvailability from '@/Components/Admin/School/SchoolModule/Select/SelectAvailability.vue'
...
// Отправка формы создания модуля
const submit = () => {
    form.transform((data) => {
    ...
    form.post(route('admin.schoolModules.store'), {
        errorBag: 'createSchoolModule',
        preserveScroll: true,
        forceFormData: true,
        // Успешное создание модуля
        onSuccess: () => toast.success('Модуль успешно создан!'),
        // Обработка ошибок формы
        onError: (errors) => {
            console.error('Ошибка создания модуля:', errors)
...
<template>
    <AdminLayout :title="t('createModule')">
        <template #header>
            <TitlePage>{{ t('createModule') }}</TitlePage>
        </template>
