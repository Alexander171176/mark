 ...
 * translations только текущей locale.
 *
 * SchoolCourseSharedResource отдаёт:
 *
 ...
 *
 * Объект для VueMultiselect каждый раз
 * вычисляется из актуальных courseOptions.
 *
 * Поэтому после ru → en уже выбранный
 * курс автоматически получает новый label.
 */
...
/**
 * Для автосинхронизации используем
 * исходные props.modules / props.lessons,
 * поскольку в Multiselect options
 * хранится только id + label.
 */
 ...
 * устанавливаем его модуль и курс.
 */
watch(
    () => form.school_lesson_id,
    (lessonId) => {
 ...
 * При выборе модуля:
 *
 * 1. автоматически устанавливаем курс;
 * 2. проверяем соответствие выбранного урока.
 */
watch(
    () => form.school_module_id,
    (moduleId) => {
...
/**
 * При смене курса проверяем,
 * соответствуют ли ему модуль и урок.
 */
watch(
    () => form.school_course_id,
    (courseId) => {
        /**
         * При очистке курса очищаем
         * всю вложенную иерархию.
         */
        if (!courseId) {
            form.school_module_id =
                null
