...
import SelectParentTrack from '@/Components/Admin/School/Track/Select/SelectParentTrack.vue'

// ======================== i18n и уведомления ========================
const { t } = useI18n()      // перевод текста интерфейса
const toast = useToast()     // всплывающие уведомления

// ======================== Props ========================
// currentLocale — текущая локаль
// availableLocales — доступные языки
// parents — список родительских треков
...
// ======================== Текущий перевод ========================
// возвращает данные текущей локали
...
// ======================== Отправка формы ========================
// отправляет форму создания трека
const submit = () => {
    form.transform((data) => {
        const transformed = {
            ...data,
    ...
    form.post(route('admin.schoolTracks.store'), {
        errorBag: 'createSchoolTrack',
        preserveScroll: true,
        forceFormData: true,
        onSuccess: () => {
...
<template>
    <AdminLayout :title="t('addLearningCategory')">
        <template #header>
            <TitlePage>{{ t('addLearningCategory') }}</TitlePage>
        </template>
