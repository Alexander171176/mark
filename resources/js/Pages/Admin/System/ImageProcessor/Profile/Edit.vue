 ...
 *
 * Редактирование Профиля обработки изображений (ImageProcessorProfile)
 */
...
/** =============================================
 * Нормализация данных ресурса
 * поддержка Resource и обычного объекта
 * ============================================= */
const profileData = props.profile?.data || props.profile
...
const submitForm = () => {
    ...
    form.put(route('admin.imageProcessorProfiles.update', {
        imageProcessorProfile: profileData.id,
    }), {
        preserveScroll: true,
        onSuccess: () => {
            toast.success('Профиль обработки изображений успешно обновлён.')
        },
        onError: (errors) => {
...
<template>
    <AdminLayout :title="t('editImageProcessingProfile')">
        <template #header>
            <TitlePage>
                {{ t('editImageProcessingProfile') }}
            </TitlePage>
        </template>
                ...
                <form @submit.prevent="submitForm" class="p-3 w-full">
                    ...
                    <div class="mb-3 flex flex-col items-start">
                        <div class="flex justify-between w-full">
                            ...
                            <div class="text-md text-gray-900 dark:text-gray-400 mt-1">
                                {{ form.description?.length || 0 }} / 500 {{ t('characters') }}
                            </div>
                        </div>
