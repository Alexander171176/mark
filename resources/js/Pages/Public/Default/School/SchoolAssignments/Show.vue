...
const assignmentTranslation = computed(() => {
    return assignmentData.value?.translation || null
})

/** Название задания */
const assignmentTitle = computed(() => {
    return assignmentTranslation.value?.title || ''
})

/** Подзаголовок задания */
const assignmentSubtitle = computed(() => {
    return assignmentTranslation.value?.subtitle || ''
})

/** Краткое описание задания */
const assignmentShort = computed(() => {
    return assignmentTranslation.value?.short || ''
})

/** Полное описание задания */
const assignmentDescription = computed(() => {
    return assignmentTranslation.value?.description || ''
})

/** Инструкции задания */
const assignmentInstructions = computed(() => {
    return assignmentTranslation.value?.instructions || ''
})
...
const seoTitle = computed(() => {
    return assignmentTranslation.value?.meta_title
        || assignmentTitle.value
})

/** SEO keywords */
const seoKeywords = computed(() => {
    return assignmentTranslation.value?.meta_keywords || ''
})

/** SEO description */
const seoDescription = computed(() => {
    return assignmentTranslation.value?.meta_description
        || assignmentShort.value
        || assignmentSubtitle.value
})

/**
 * Локаль фактически выбранного backend-перевода.
 *
 * Никаких fallback-языков Vue не знает.
 */
const resolvedLocale = computed(() => {
    return assignmentTranslation.value?.locale
        || props.locale
        || ''
})
...
const assignmentImages = computed(() => {
    return Array.isArray(assignmentData.value?.images)
        ? assignmentData.value.images
        : []
})
...
const courseData = computed(() => {
    return assignmentData.value?.course || null
})

/** Модуль */
const moduleData = computed(() => {
    return assignmentData.value?.module || null
})
...
const courseTitle = computed(() => {
    return courseData.value?.translation?.title
        || t('course')
})

/** Название модуля */
const moduleTitle = computed(() => {
    return moduleData.value?.translation?.title
        || t('module')
})

/** Название урока */
const lessonTitle = computed(() => {
    return lessonData.value?.translation?.title
        || t('lesson')
})

/** Имя инструктора */
const instructorName = computed(() => {
    return instructorData.value?.user?.name
        || instructorData.value?.translation?.title
        || t('instructor')
})
...
const formatDate = (value) => {
    ...
    return new Intl.DateTimeFormat(
        resolvedLocale.value || undefined,
        {
            day: '2-digit',
            month: '2-digit',
            year: 'numeric',
...
<template>
    ...
    <Head>
        ...
        <meta
            property="og:url"
            :content="`/school/assignments/${assignmentData.slug || ''}`"
        />

        <meta
            property="og:image"
            :content="firstAssignmentImageUrl"
        />
    ...
    >
        ...
        <div class="min-h-screen px-1.5">
            <main class="mx-auto flex flex-col lg:flex-row gap-4 tracking-wider">
                ...
                <section class="w-full lg:mt-28 pb-6 slate-1 min-w-0">
                    <div class="mx-auto max-w-6xl">
                        <article class="selection:bg-red-400 selection:text-white">
                            ...
                            <!-- Данные -->
                            <div
                                class="my-4 flex flex-wrap items-center
                            ...
                            >
                                ...
                                <Link
                                    ...
                                    class="rounded-sm border border-gray-400
                                           flex items-center justify-center
                                           gap-1 px-3 py-1 hover:text-blue-600
                                ...
                                >
                                    <svg
                                        class="shrink-0 h-3 w-3
                                               text-indigo-600/85
                                               dark:text-indigo-300/85"
                                        fill="currentColor"
                                        viewBox="0 0 24 24"
                                    >
                                        <path
                                            d="M7 4.75h8.5A2.75 2.75 0 0 1 18.25 7.5v9A2.75 2.75 0 0 1 15.5 19.25H7A2.25 2.25 0 0 1 4.75 17V7A2.25 2.25 0 0 1 7 4.75Z"
                                        />
                                        <path
                                ...
                                >
                                    ...
                                    {{ t('status') }}:
                                    {{ translateStatus(assignmentData.status) }}
                                </span>
                                ...
                                >
                                    ...
                                    {{ t('gradingType') }}:
                                    {{ translateGradingType(
                                    assignmentData.grading_type
                                ) }}
                                </span>

                                <!-- Максимальная оценка -->
                                <span
                                    v-if="assignmentData.max_score !== null
                                        && assignmentData.max_score !== undefined"
                                    class="rounded-sm border border-gray-400
                                           flex items-center justify-center
                                           gap-1 px-3 py-1"
                                >
                                ...
                                >
                                    ...
                                    >
                                        <path
                                            d="M12 2a10 10 0 1 0 10 10A10.01 10.01 0 0 0 12 2Zm1 5h-2v6l5 3 1-1.73-4-2.27Z"
                                        />
                                    </svg>

                                    {{ t('attemptsLimit') }}:
                                    {{
                                        assignmentData.attempts_limit === 0
                                            ? t('no')
                                            : assignmentData.attempts_limit
                                    }}
                                </span>
                            ...
                            <!-- Описание -->
                            <div
                                v-if="assignmentDescription"
                                class="mt-4 text-sm subtitle"
                                v-html="assignmentDescription"
                            />

                            <!-- Инструкции -->
                            <div
                                v-if="assignmentInstructions"
                                class="mt-6"
                            >
                                <h2
                ...
                <aside
                    v-if="showRight"
                    class="shrink-0 lg:mt-28 pr-3
                           transition-all duration-300"
                    :class="rightCollapsed
                        ? 'lg:w-10'
                        : 'lg:w-64'"
                >
                    <RightSidebarSchool
