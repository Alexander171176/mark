...
/** Универсальный список инструкторов */
 ...
 * Оба свернуты → 4.
 *
 * Количество инструкторов при этом не меняется.
 */
...
/**
 * Количество инструкторов на странице.
 *
        ...
        switch (sort.value) {
            ...
            case 'nameAsc':
                return normalizeText(getInstructorTitle(a))
                    .localeCompare(
                        normalizeText(getInstructorTitle(b))
                    )
            ...
            case 'nameDesc':
                return normalizeText(getInstructorTitle(b))
                    .localeCompare(
                        normalizeText(getInstructorTitle(a))
                    )
            ...
            case 'experienceAsc':
                return (a.experience_years ?? 0)
                    - (b.experience_years ?? 0)

            case 'experienceDesc':
                return (b.experience_years ?? 0)
...
const currentPage = computed(() => {
    return Number(
        ...
        ?? props.instructorProfiles?.current_page
        ?? 1
    ) || 1
})
...
const lastPage = computed(() => {
    return Number(
        ...
        ?? props.instructorProfiles?.last_page
        ?? 1
    ) || 1
})

/** Маршрут списка инструкторов */
const indexRoute = () => {
    return route('public.schoolInstructors.index')
}

/**
 * Server-загрузка инструкторов.
 *
